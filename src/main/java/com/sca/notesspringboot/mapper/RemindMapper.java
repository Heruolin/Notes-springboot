...
public interface RemindMapper extends BaseMapper<Remind> {
    ...
    @Insert("INSERT INTO `archive-remind` (id, userid, text, remind_time, `order`) " +
            "SELECT id, userid, text, remind_time, `order` FROM remind WHERE id = #{id}")
    ...
    // 从归档插入到 remind 表
    @Insert("INSERT INTO remind (id, userid, text, remind_time, `order`) " +
            "SELECT id, userid, text, remind_time, `order` FROM `archive-remind` WHERE id = #{id}")
    ...
    // 插入到 trash-remind 表
    @Insert("INSERT INTO `trash-remind` (id, userid, text, remind_time, `order`, trash_time) " +
            "SELECT id, userid, text, remind_time, `order`, NOW() FROM remind WHERE id = #{id}")
    ...
    // 从回收站插入到 remind 表
    @Insert("INSERT INTO remind (id, userid, text, remind_time, `order`) " +
            "SELECT id, userid, text, remind_time, `order` FROM `trash-remind` WHERE id = #{id}")
